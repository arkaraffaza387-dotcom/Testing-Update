--[[
    ZetGames-AimLock-Advanserver V4.3 | TESTING BUILD
    Theme: Red & Black Hacker Style
    Features: Fly Normal + Theme Switcher + Config Save/Load + NPC Detection + NPC ESP + NPC Filter
...
local FlyNormalMode = "Free"  -- "Free" / "Hover"
...
ScreenGui.Name = "ZetGamesAdvanserverV43"
...
local function IsNPC(model)
    ...
    -- Cek whitelist keyword (jangan lock kalau nama ada keyword tertentu)
    ...
    -- Kalau ada filter nama, harus match
...
local function EnableFlyNormal()
    ...
    FlyNormalConnection = RunService.RenderStepped:Connect(function()
        if not FlyNormalEnabled or not IsLoggedIn then return end
        ...
        -- Mobile: pakai Humanoid.MoveDirection
...
local function DoServerHop()
    if ServerHopRunning then Notify("Server Hop", "> SEDANG PROSES...", 2); return end
...
local function EnableFlyVoid()
    ...
    local ray = workspace:Raycast(
        Vector3.new(root.Position.X, 1000, root.Position.Z),
...
local function UpdateNPCEsp()
    if not NPCEspEnabled or not IsLoggedIn then
...
local function FindTarget()
    ...
    -- 🆕 Loop NPC (kalau NPC Detection ON)
...
local function RunAimbot()
    ...
    if not AimbotStickyTarget then
        ...
        AimbotStickyTarget = t
        AimbotStickyType = tType
    end
...
local function CreateRadarUI()
    ...
    RadarFrame.Visible = false    RadarFrame.Parent = ScreenGui
...
local function StartRadar()
    ...
    RadarConnection = RunService.RenderStepped:Connect(function()
        ...
        -- 🆕 NPC di Radar (kalau NPC Detection ON)
        if NPCDetectionEnabled then
            for _, model in ipairs(GetNPCs()) do
                local root = model:FindFirstChild("HumanoidRootPart")
                if root then
...
local function SaveConfig()
    ...
    cfg.esp = ESPEnabled
    cfg.rainbow = RainbowESPEnabled
    cfg.radar = RadarEnabled
    cfg.radarradius = RadarRadius
    cfg.radarzoom = RadarZoom
    cfg.theme = CurrentThemeName
    cfg.flynormal = FlyNormalEnabled
    cfg.flyspeed = FlyNormalSpeed
    cfg.flymode = FlyNormalMode
    cfg.speed = SpeedHackEnabled
    cfg.speedval = SpeedMultiplier
    cfg.hitbox = HitboxEnabled
    cfg.hitboxsize = HitboxSize
    cfg.antiafk = AntiAFKEnabled
    cfg.fullbright = FullbrightEnabled
    cfg.fpsboost = FPSBoostEnabled
    cfg.noclip = NoclipEnabled
    cfg.infinitejump = InfiniteJumpEnabled
...
local function LoadConfig(jsonStr)
    ...
    if not ok or not cfg then Notify("Config", "> INVALID JSON", 2); return false end
    ...
    if cfg.rainbow ~= nil then
        RainbowESPEnabled = cfg.rainbow
        if RainbowESPEnabled then EnableRainbowESP() else DisableRainbowESP() end
    end
    if cfg.radarradius then RadarRadius = cfg.radarradius end
    if cfg.radarzoom then RadarZoom = cfg.radarzoom end
    if cfg.theme then
        CurrentThemeName = cfg.theme
        -- Apply theme
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if input.KeyCode == FlyNormalKeybind and IsLoggedIn then
        FlyNormalEnabled = not FlyNormalEnabled
        if FlyNormalEnabled then
            EnableFlyNormal()
            Notify("Fly Normal", "> ACTIVE (F)", 2)
        else
            DisableFlyNormal()
            Notify("Fly Normal", "> OFF", 2)
        end
    end
    -- Fly Normal movement keys
...
local function ApplyTheme(themeName)
    ...
    for _, ref in pairs(UIReferences) do
        pcall(function()
            if ref.Type == "Main" then
                ref.Object.BackgroundColor3 = THEME.MainBG
                ref.Object.BorderColor3 = THEME.AccentColor
            elseif ref.Type == "Section" then
                ref.Object.BackgroundColor3 = THEME.SectionBG
                ref.Object.BorderColor3 = THEME.AccentColor
            elseif ref.Type == "Panel" then
                ref.Object.BackgroundColor3 = THEME.PanelBG
                ref.Object.BorderColor3 = THEME.AccentColor
            elseif ref.Type == "Text" then
                ref.Object.TextColor3 = THEME.TextColor
            elseif ref.Type == "TextLight" then
                ref.Object.TextColor3 = THEME.TextLight
            end
        end)
    end
...
local function CreateUI()
    ...
    LFooterLbl.Text = "> NPC + FLY + THEME + CONFIG"
    ...
    LTopTxt.Text = "● V4.3 TESTING [MOBILE]"
    ...
    LTitle2.Position = UDim2.new(0, 15, 0, 45)
    ...
    LTag2.Position = UDim2.new(0, 15, 0, 78)
    LTag2.BackgroundTransparency = 1
    LTag2.Text = "[ MOBILE EDITION - NO NIGHT LOCK ]"
    ...
    KeyLbl.Position = UDim2.new(0, 15, 0, 105)
    ...
    LoginBtn.Size = UDim2.new(1, -30, 0, 42)
    LoginBtn.Position = UDim2.new(0, 15, 0, 183)
    ...
    GetKeyBtn.Size = UDim2.new(1, -30, 0, 42)
    ...
    StatusTxt.Position = UDim2.new(0, 15, 0, 297)
    ...
    MainTitle2.Text = "● V4.3 TESTING [MOBILE]"
    ...
    ScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 4600)
    ScrollFrame.ZIndex = 11
    ScrollFrame.Parent = MainHub

    local ScrollContent = Instance.new("Frame")
    ScrollContent.Size = UDim2.new(1, 0, 0, 4600)
    ...
    BuildLbl.Text = "> BUILD: V4.3 TESTING"
    ...
    -- 2. 🎨 THEME SWITCHER
    --==========================================================
    Section("=== 🎨 THEME SWITCHER ===", 135)

    local ThemeLbl = Instance.new("TextLabel")
    ThemeLbl.Size = UDim2.new(1, -20, 0, 20)
    ThemeLbl.Position = UDim2.new(0, 10, 0, 167)
    ...
    ThemeBtnFrame.Position = UDim2.new(0, 10, 0, 190)
    ...
    Half("> 💾 SAVE", 335, 0, function(btn)
        local cfgStr = SaveConfig()
        ConfigOutput.Text = cfgStr
        if setclipboard then
            setclipboard(cfgStr)
            Notify("Config", "> SAVED + COPIED", 3)
        else
            Notify("Config", "> SAVED (copy manual)", 3)
        end
    end)

    Half("> 📂 LOAD", 335, 0.5, function(btn)
        local cfgStr = ConfigOutput.Text
        if cfgStr ~= "" then
            LoadConfig(cfgStr)
        else
            Notify("Config", "> PASTE CONFIG DULU", 2)
        end
    end)
    ...
    -- 4. 🛫 FLY NORMAL (SKY)
    --==========================================================
    Section("=== 🛫 FLY NORMAL (SKY) ===", 380)

    Toggle("> FLY NORMAL: OFF (F)", 412, function(btn)
        FlyNormalEnabled = not FlyNormalEnabled
        if FlyNormalEnabled then
    ...
    Half("> MODE: FREE", 454, 0, function(btn)
        FlyNormalMode = FlyNormalMode == "Free" and "Hover" or "Free"
        btn.Text = "> MODE: " .. string.upper(FlyNormalMode)
    end)

    Half("> KEY: F", 454, 0.5, function(btn)
        Notify("Fly Normal", "> Tekan F untuk ON/OFF", 2)
    end)

    local FlySpeedInput = Input("> Speed (10-500)", 491, tostring(FlyNormalSpeed))
    ...
    --==========================================================
    -- 5. NPC DETECTION & FILTER
    --==========================================================
    Section("=== 🆕 NPC DETECTION ===", 540)

    Toggle("> NPC DETECTION (AIMBOT): OFF", 572, function(btn)
        ...
        Notify("NPC", NPCDetectionEnabled and "> Aimbot lock NPC" or "> NPC OFF", 2)
    end)

    Toggle("> NPC ESP: OFF", 614, function(btn)
    ...
    local NPCFilterInput = Input("> Filter nama NPC (kosong=all)", 656, NPCFilterName)
    NPCFilterInput.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            NPCFilterName = NPCFilterInput.Text
            Notify("NPC Filter", "> Name: " .. (NPCFilterName ~= "" and NPCFilterName or "ALL"), 2)
        end
    end)

    local NPCMaxDistInput = Input("> Max Distance (50-5000)", 693, tostring(NPCMaxDistance))
    ...
    local NPCMaxCountInput = Input("> Max NPC Count (1-200)", 730, tostring(NPCMaxCount))
    ...
    task.spawn(function()
        while task.wait(2) do
            if IsLoggedIn then pcall(RefreshTeleportList) end
        end
    end)
    ...
    -- 7. SERVER HOP
    --==========================================================
    Section("=== 🌐 SERVER HOP ===", 1005)

    Toggle("> 🌐 SERVER HOP (RANDOM)", 1037, function(btn) DoServerHop() end)
    Half("> 🎯 BEST SERVER", 1079, 0, function(btn) DoBestServerHop() end)
    Half("> 🔄 REJOIN", 1079, 0.5, function(btn) DoRejoin() end)

    local JobIdInput = Input("> Paste Job ID (fallback)", 1116)
    Toggle("> 🔗 JOIN BY JOB ID", 1153, function(btn)
        DoJoinByJobId(JobIdInput.Text)
    end)

    --==========================================================
    -- 8. MAIN FEATURES
    --==========================================================
    Section("=== MAIN FEATURES ===", 1202)

    Toggle("> FPS BOOST: OFF", 1234, function(btn)
    ...
    Toggle("> FULLBRIGHT: OFF", 1276, function(btn)
    ...
    Toggle("> SPEED HACK: OFF", 1318, function(btn)
    ...
    local SpeedInput = Input("> Speed (16-500)", 1360, tostring(SpeedMultiplier))
    ...
    Toggle("> INFINITE JUMP: OFF", 1397, function(btn)
    ...
    Toggle("> NOCLIP: OFF", 1439, function(btn)
    ...
    -- 9. HITBOX EXPANDER
    --==========================================================
    Section("=== 🎯 HITBOX EXPANDER ===", 1487)

    Toggle("> HITBOX: OFF", 1519, function(btn)
        HitboxEnabled = not HitboxEnabled
        if HitboxEnabled then
            btn.Text = "> HITBOX: ON"
            btn.BackgroundColor3 = THEME.ButtonActive
            StartHitboxExpander()
            Notify("Hitbox", "> ACTIVE", 2)
        else
            btn.Text = "> HITBOX: OFF"
            btn.BackgroundColor3 = THEME.ButtonBG
            StopHitboxExpander()
    ...
    local HitboxInput = Input("> Size (1-1000)", 1561, tostring(HitboxSize))
    ...
    -- 10. ANTI-AFK
    --==========================================================
    Section("=== 😴 ANTI-AFK ===", 1608)

    Toggle("> ANTI-AFK: OFF", 1640, function(btn)
    ...
    --==========================================================
    Section("=== SURVIVAL FEATURES ===", 1688)

    Toggle("> AUTO RESPAWN: OFF", 1720, function(btn)
    ...
    Toggle("> ANTI-FLING: OFF", 1762, function(btn)
    ...
    Toggle("> LOOK AT PLAYER: OFF", 1804, function(btn)
    ...
    -- 12. FLY-VOID V2
    --==========================================================
    Section("=== 🕳️ FLY-VOID V2 ===", 1853)

    Toggle("> FLY-VOID: OFF (V)", 1885, function(btn)
        FlyVoidEnabled = not FlyVoidEnabled
        if FlyVoidEnabled then
            btn.Text = "> FLY-VOID: ON"
            btn.BackgroundColor3 = THEME.ButtonActive
            EnableFlyVoid()
            Notify("Fly-Void V2", "> ACTIVE", 2)
        else
    ...
    Half("> HIDE: OFF", 1926, 0, function(btn)
    ...
    Half("> KEY: V", 1926, 0.5, function(btn)
    ...
    -- 13. SOUND ESP
    --==========================================================
    Section("=== SOUND ESP ===", 1973)

    Toggle("> SOUND ESP: OFF", 2005, function(btn)
    ...
    local SndInput = Input("> Radius (10-500)", 2047, tostring(SoundESPRadius))
    ...
    Toggle("> MUSIC PLAYER: OFF", 2153, function(btn)
    ...
    Half("> ⏮ PREV", 2195, 0, function(btn) PrevMusic() end)
    Half("> ⏭ NEXT", 2195, 0.5, function(btn) NextMusic() end)

    local MusicVolInput = Input("> Volume (0-10)", 2232, tostring(MusicVolume))
    ...
    local AddNameInput = Input("> Nama lagu baru", 2269)
    local AddIDInput = Input("> Sound ID lagu baru", 2306)
    Toggle("> ➕ ADD LAGU", 2343, function(btn)
    ...
    -- 15. ENHANCED AIMBOT
    --==========================================================
    Section("=== 🎯 ENHANCED AIMBOT ===", 2399)

    local AimbotBtn = Instance.new("TextButton")
    AimbotBtn.Size = UDim2.new(1, -20, 0, 45)
    AimbotBtn.Position = UDim2.new(0, 10, 0, 2431)
    ...
    AimbotInfo.Position = UDim2.new(0, 10, 0, 2480)
    AimbotInfo.BackgroundTransparency = 1
    AimbotInfo.Text = "> ON = auto lock Player + NPC (kalau ON)"
    ...
    Half("> FOV CIRCLE: OFF", 2510, 0, function(btn)
    ...
    Half("> TEAM CHECK: OFF", 2510, 0.5, function(btn)
    ...
    TargetLbl.Position = UDim2.new(0, 10, 0, 2552)
    ...
    TargetBtnFrame.Position = UDim2.new(0, 10, 0, 2572)
    ...
    local FOVInput = Input("> FOV Radius (50-2000)", 2609, tostring(AimbotFOV))
    ...
    Toggle("> KEYBIND HOLD (E): OFF", 2646, function(btn)
    ...
    Toggle("> WALL CHECK: OFF", 2688, function(btn)
    ...
    -- 16. FULL ESP
    --==========================================================
    Section("=== FULL ESP ===", 2735)

    Toggle("> ESP MASTER: OFF", 2767, function(btn)
    ...
    ESPInfo.Position = UDim2.new(0, 10, 0, 2809)
    ESPInfo.BackgroundTransparency = 1
    ESPInfo.Text = "> ON = Box + Name + Dist + HP + Skeleton\n> (NPC ESP terpisah — warna kuning)"
    ...
    Toggle("> 🌈 RAINBOW ESP: OFF", 2857, function(btn)
    ...
    -- 17. RADAR BULAT
    --==========================================================
    Section("=== 📡 RADAR BULAT ===", 2905)

    Toggle("> RADAR: OFF", 2937, function(btn)
    ...
    Half("> ROTATE: OFF", 2979, 0, function(btn)
    ...
    Half("> NAMES: OFF", 2979, 0.5, function(btn)
    ...
    local RadarRadInput = Input("> Radius (100-1000)", 3016, tostring(RadarRadius))
    ...
    local RadarZoomInput = Input("> Zoom (0.5-3.0)", 3053, tostring(RadarZoom))
    ...
    -- 18. CHAT SPAM
    --==========================================================
    Section("=== CHAT SPAM ===", 3101)

    Toggle("> CHAT SPAM: OFF", 3133, function(btn)
    ...
    local ChatInput = Input("> Message", 3175, ChatSpamText)
    ...
    -- 19. TELEPORT
    --==========================================================
    Section("=== TELEPORT ===", 3222)

    Toggle("> TELEPORT TO MOUSE", 3254, function(btn) TeleportToMouse() end)
    Half("> SAVE LOC", 3296, 0, function(btn) SaveLocation() end)
    Half("> LOAD LOC", 3296, 0.5, function(btn) LoadLocation() end)
    ...
    LoginBtn.MouseButton1Click:Connect(function()
        ...
        if keyData then
            if keyData.Expiry == 0 or os.time() < keyData.Expiry then
                ...
                Notify("New", "> NPC + Fly Normal + Theme + Config", 3)
                task.wait(0.5)
                RefreshTeleportList()
            else
    ...
    task.spawn(function()
        ...
        Notify("V4.3 TESTING", "> NPC + FLY + THEME + CONFIG", 3)
